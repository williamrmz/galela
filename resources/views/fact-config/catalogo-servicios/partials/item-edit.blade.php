...
{{  Form::open(['route' => ['fact-config.catalogo-servicios.update', $item->IdProducto], 'method'=>'PUT', 'id'=>$model.'-form']) }}
	<div class="row">
						...
						<div class="col-sm-4">
							<table class="" style="width:100%;">
								<tr>
									...
									<td>{{ Form::text('codigo', $item->Codigo, ['class'=>'form-control input-sm']) }}</td>
								</tr>
								<tr>
									<?php $tipoServicio = $item->EsCPT? 'S': 'I' ?>
									<td>{{ Form::label('tipoServicio', 'Servicio') }}</td>
									<td>{{ Form::select('tipoServicio', ['S'=>'Procedimiento CPT', 'I'=>'Insumo'], $tipoServicio, ['class'=>'form-control input-sm']) }}</td>
								</tr>
								<tr>
									<?php $estado = $item->idEstado? true: false;?>
									<td>{{ Form::label('estado', 'Estado') }}</td>
									<td>{{ Form::checkbox('estado', 1, $estado) }}</td>
								</tr>
							</table>
						</div>
						<div class="col-sm-8">
							<table style="width:100%;">
								<tr>
									...
									<td>{{ Form::text('nombre', $item->Nombre, ['class'=>'form-control input-sm']) }}</td>
								</tr>
								<tr>
									<td width="100">{{ Form::label('nombreMinsa', 'Nombre MINSA') }}</td>
									<td>{{ Form::text('nombreMinsa', $item->NombreMINSA, ['class'=>'form-control input-sm']) }}</td>
								</tr>
								<tr>
									<td width="100">{{ Form::label('codigoSis', 'Codigo SIS') }}</td>
									<td>{{ Form::text('codigoSis', $item->codigoSIS, ['class'=>'form-control input-sm']) }}</td>
								</tr>
							</table>
						</div>
					</div>
		...
		<div class="col-sm-12">
			<div class="pull-right">
				...
				<button type="submit" class="btn btn-sm btn-success {{$model}}-btn-update">ACTAULIZAR</button>
			</div>
		</div>
	</div>
{{ Form::close() }}
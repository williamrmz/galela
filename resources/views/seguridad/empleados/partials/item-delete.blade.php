...
{{  Form::open(['route' => ['seguridad.empleados.destroy', $item->IdEmpleado], 'method'=>'DELETE', 'id'=>$model.'-form', 'autocomplete'=>'off']) }}
	<div class="row">
		<div class="col-sm-12">
					...
					<div class="box box-primary">
						...
						<div class="box-body">
							<div class="row">
								...
								<div class="col-sm-8"> {{ Form::text('codigoPlanilla', null, ['class'=>'form-control', 'readonly']) }} </div>

								<div class="col-sm-4"> {{ Form::label('idTipoDocumento', 'Documento') }} </div>
								<div class="col-sm-8"> 
									<div class="input-group">
										<span class="input-group-addon" style="padding:0; border:0;">
											{{ Form::select('idTipoDocumento', [], null, ['class'=>'form-control ', 'style'=>'width:150px;', 'disabled']) }}
										</span>
										{{ Form::text('dni', null, ['class'=>'form-control', 'id'=>'dni', 'readonly'] ) }}
									</div>
									
								</div>
								
								<div class="col-sm-4"> {{ Form::label('apellidoPaterno', 'Apellido paterno') }}</div>
								<div class="col-sm-8"> {{ Form::text('apellidoPaterno', null, ['class'=>'form-control', 'readonly']) }} </div>

								<div class="col-sm-4"> {{ Form::label('apellidoMaterno', 'Apellido materno') }}</div>
								<div class="col-sm-8"> {{ Form::text('apellidoMaterno', null, ['class'=>'form-control', 'readonly']) }} </div>

								<div class="col-sm-4"> {{ Form::label('nombres', 'Nombres') }}</div>
								<div class="col-sm-8"> {{ Form::text('nombres', null, ['class'=>'form-control', 'readonly']) }} </div>

								<div class="col-sm-4"> {{ Form::label('fechaNacimiento', 'Fecha nacimiento') }}</div>
								<div class="col-sm-8"> {{ Form::date('fechaNacimiento', null, ['class'=>'form-control', 'readonly']) }} </div>

								<div class="col-sm-4"> {{ Form::label('idTipoEmpleado', 'Tipo empleado') }}</div>
								<div class="col-sm-8"> {{ Form::select('idTipoEmpleado', [], null, ['class'=>'form-control', 'style'=>'width:100%;', 'disabled']) }}  </div>

								<div class="col-sm-4"> {{ Form::label('idCondicionTrabajo', 'Condicion trabajo') }}</div>
								<div class="col-sm-8"> {{ Form::select('idCondicionTrabajo', [], null, ['class'=>'form-control', 'style'=>'width:100%;', 'disabled']) }}  </div>

								<div class="col-sm-4"> {{ Form::label('idTipoDestacado', 'Tipo destacado') }}</div>
								<div class="col-sm-8"> {{ Form::select('idTipoDestacado', [], null, ['class'=>'form-control', 'style'=>'width:100%;', 'disabled']) }}  </div>

								<div class="col-sm-4"> {{ Form::label('idSupervisor', 'Supervisor (Galen)') }}</div>
								<div class="col-sm-8"> {{ Form::text('idSupervisor', null, ['class'=>'form-control', 'readonly']) }} </div>
								
								<div class="col-sm-4"> {{ Form::label('idEstablecimientoExterno', 'Cs, Ps externo donde labora (HisGalenhos)') }}</div>
								<div class="col-sm-8"> {{ Form::text('idEstablecimientoExterno', null, ['class'=>'form-control', 'readonly']) }} </div>
							...
							<div class="nav-tabs-custom">
								...
								<div class="tab-content">
									<div class="tab-pane active" id="tab_1">
										<div class="row">
											<div class="col-sm-10">
												{{ Form::select('lista-roles', [], null, ['class'=>'form-control lista-roles', 'style'=>'width:100%', 'disabled']) }}
											</div>
											<div class="col-sm-2">
									...
									<div class="tab-pane" id="tab_2">
										<div class="row">
											<div class="col-sm-10">
												{{ Form::select('lista-cargos', [], null, ['class'=>'form-control lista-cargos', 'style'=>'width:100%', 'disabled']) }}
											</div>
											<div class="col-sm-2">
									...
									<div class="tab-pane" id="tab_3">
										<div class="row">
											<div class="col-sm-5">
												{{ Form::select('lista-areas', [], null, ['class'=>'form-control lista-areas', 'style'=>'width:100%', 'disabled']) }}
											</div>
							...
							<div class="box box-primary">
								...
								<div class="box-body">
									<div class="row">
										...
										<div class="col-sm-7"> {{ Form::text('usuario', null, ['class'=>'form-control input-sm', 'readonly']) }} </div>

										<div class="col-sm-5"> {{ Form::label('clave', 'Clave') }}</div>
										<div class="col-sm-7"> {{ Form::password('clave',  ['class'=>'form-control input-sm', 'readonly']) }} </div>

										<div class="col-sm-5"> {{ Form::label('usaGalenhos', 'Esta usando Galenhos') }}</div>
										<div class="col-sm-7">
											<div class="input-group">
												<span class="input-group-addon"> {{ Form::checkbox('usaGalenhos', 1, false, ['disabled'])}} </span>
												{{ Form::text('loginPc', null, ['class'=>'form-control input-sm ', 'id'=>'loginPc', 'readonly']) }}
											</div>
										</div>

										<div class="col-sm-5"> {{ Form::label('codigoDigitador', 'Codigo digitador (HIS)') }}</div>
										<div class="col-sm-7"> {{ Form::text('codigoDigitador', null, ['class'=>'form-control input-sm', 'readonly'])}} </div>

										<div class="col-sm-5"> {{ Form::label('reniecAutorizado', 'Autorizado por RENIEC') }}</div>
										<div class="col-sm-7"> {{ Form::checkbox('reniecAutorizado', 1, false, ['style'=>'margin-left: 13px', 'disabled'])}} </div>
									</div>
								</div>
							</div>
